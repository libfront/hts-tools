const l = async (a) => ({
  set: async (s, d) => {
    console.log(`--renderer [${s.data.curr.type}]`);
    const r = {
      value: () => s.data.curr.data.data,
      formattedJson: () => {
        try {
          const e = JSON.parse(s.data.curr.data.data || "{}");
          let t = JSON.stringify(e, null, 2);
          return t = t.replace(/\{\n\s+"/, `{
  "`), t;
        } catch {
          return s.data.curr.data.data || "{}";
        }
      },
      theme: () => s.data.curr.data.theme || "light"
    };
    return {
      r: (() => {
        const e = r.theme(), t = (o) => o.replace(/"/g, "&quot;");
        return `<div id="${a.f.name("root")}" class="min-h-screen transition-colors duration-300 ${e === "dark" ? "bg-gray-900 text-white" : "bg-gray-50 text-gray-900"}" 
